...
*   **[Icarus Verilog](iverilog.icarus.com):** The compiler and simulator used to process the Verilog code and generate the value change dump (`.vcd`) files.
*   **[GTKWave](gtkwave.sourceforge.net):** The waveform viewer used to visually analyze the simulation results and verify the correct functionality of the circuits.
...
*   `[circuit_name]_tb.v`: The corresponding testbench for verification.
...
3.  **Compile** the testbench using `iverilog`:
    ```bash
    iverilog -o waveform.vvp [circuit_name]_tb.v
    ```
